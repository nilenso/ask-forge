...
describe("forge", () => {
  ...
  beforeAll(async () => {
    ...
    testDir = join(
      tmpdir(),
      `ask-forge-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe("connectRepo", () => {
    ...
    test("reuses cached bare repo for different commitish", async () => {
      ...
      const readme1 = await readFile(
        join(repo1.localPath, "README.md"),
        "utf-8",
      );
      const readme2 = await readFile(
        join(repo2.localPath, "README.md"),
        "utf-8",
      );
    ...
    test("throws for unknown forge without explicit option", async () => {
      expect(
        connectRepo("https://unknown-forge.com/user/repo"),
  ...
  describe("cleanupWorktree", () => {
    test("removes worktree successfully", async () => {
      ...
      expect(
        readFile(join(repo.localPath, "README.md"), "utf-8"),
      ).rejects.toThrow();
    });
