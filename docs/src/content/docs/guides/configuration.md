...
For the full event reference and consumption patterns (mixing iteration with `.result()`, error events, tool events, compaction events), see [Handling Responses](/megasthenes/guides/handling-responses/).
...
- `afterTurn` — branch from a specific turn. See [Multi-turn Conversations — Conversation Branching](/megasthenes/guides/multi-turn-conversations/#conversation-branching).
