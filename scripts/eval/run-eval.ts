...
function parseCsv(content: string): ParseResult {
  ...
  if (!hasSessionId && !hasId) {
    return {
      ok: false,
      error:
        'Missing identifier column: expected either "session_id" or "id"',
    };
  }

  // Build column index map so column order doesn't matter
  const colIndex = Object.fromEntries(
    header.map((col, idx) => [col.trim(), idx]),
...
async function runEval(inputPath: string): Promise<void> {
  ...
  type RowKey = string;
  const makeKey = (r: EvalRow): RowKey =>
    `${r.repository}::${r.commit_id}::${r.question}`;

  // Group unique questions by (repo, commit) so we can reuse a single session per repo+commit
  type RepoKey = string;
  const makeRepoKey = (r: { repository: string; commit_id: string }): RepoKey =>
    `${r.repository}::${r.commit_id}`;

  const questionsByRepo = new Map<
    RepoKey,
    { repository: string; commit_id: string; questions: Map<RowKey, string> }
  >();
  ...
  const totalQuestions = [...questionsByRepo.values()].reduce(
    (sum, g) => sum + g.questions.size,
    0,
  );
  ...
  const results = new Map<
    RowKey,
    { answer: string; toolCallCount: number; inferenceTimeMs: number; judge: JudgeResult | null }
  >();
  ...
  for (const [, { repository, commit_id, questions }] of questionsByRepo) {
    ...
    try {
      ...
      for (const [rowKey] of questions) {
        results.set(rowKey, { answer: "", toolCallCount: 0, inferenceTimeMs: 0, judge: null });
      }
      continue;
    }
    ...
    for (const [rowKey, question] of questions) {
      ...
      try {
        ...
        const secs = (inferenceTimeMs / 1000).toFixed(1);
        console.log(
          `  ✓ Got response (${answer.length} chars, ${toolCallCount} tool calls, ${secs}s)`,
        );
      } catch (error) {
        console.error(
          `  ✗ Ask error: ${error instanceof Error ? error.message : String(error)}`,
        );
        results.set(rowKey, { answer: "", toolCallCount: 0, inferenceTimeMs: 0, judge: null });
        continue;
      }
      ...
      try {
        ...
        console.log(
          `  ✓ Judge: relevant=${judgeResult.is_answer_relevant} evidence=${judgeResult.is_evidence_supported} linked=${judgeResult.is_evidence_linked}`,
        );
      } catch (error) {
        console.error(
      ...
      results.set(rowKey, { answer, toolCallCount, inferenceTimeMs, judge: judgeResult });
    }
  ...
  const resultRows: EvalRow[] = rows.map((row) => {
    ...
    return {
      ...
      is_evidence_linked: result?.judge?.is_evidence_linked ?? "",
  ...
  console.log(
    `Answer relevant:     ${relevant}/${total} (${((relevant / total) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Evidence supported:  ${evidenced}/${total} (${((evidenced / total) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Evidence linked:     ${linked}/${total} (${((linked / total) * 100).toFixed(1)}%)`,
  );
  ...
  const reportHtml = await generateReport(
    output,
    { total, relevant, evidenced, linked },
    timestamp,
  );
